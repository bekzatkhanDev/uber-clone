...
const DriverRegister = () => {
  ...
  const loading = isRegistering || isLoggingIn || isCreatingProfile || isAddingCar;
        ...
        onSuccess: () => {
          login(
            { phone: account.phone, password: account.password },
            {
              ...
              onError: () => Alert.alert(t.common.error, t.auth.loginFailed),
            }
          );
        },
        onError: (err: any) => {
  ...
  const handleStep2 = () => {
    ...
    if (isNaN(years) || years < 0) {
      Alert.alert(t.common.error, t.auth.experienceYears);
      return;
    }
    createProfile(
      { license_number: profile.license_number, experience_years: years },
      {
        onSuccess: () => setStep(3),
        onError: (err: any) => {
          const message = err?.responseData?.license_number?.[0]
            ?? err?.message
  ...
  const handleStep3 = () => {
    ...
    if (isNaN(year) || year < 1990 || year > new Date().getFullYear() + 1) {
      Alert.alert(t.common.error, t.auth.carYear);
      return;
    }
    addCar(
      { brand_id: car.brand_id, car_type_id: car.car_type_id, year, plate_number: car.plate_number },
      {
        ...
        onError: (err: any) => {
          const message = err?.responseData?.plate_number?.[0]
            ?? err?.message
            ?? t.auth.registrationFailed;
          Alert.alert(t.common.error, message);
        },
        ...
        <View className="p-5">
          ...
          {step === 1 && (
            <>
              ...
              />
              <CustomButton
                title={isRegistering || isLoggingIn ? t.auth.registering : t.auth.continue}
                onPress={handleStep1}
                className="mt-6"
